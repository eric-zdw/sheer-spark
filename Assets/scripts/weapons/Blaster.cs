...
public class Blaster : Weapon {
    ...
    public override void Fire1()
    {
        if (GetCooldown() <= 0)
        {
            Instantiate(projectile, transform.position + (Vector3.Normalize((Vector3)mousePosition - transform.position) * 0.5f), Quaternion.Euler(0, 0, angle + Random.Range(-2f, 2f)));
